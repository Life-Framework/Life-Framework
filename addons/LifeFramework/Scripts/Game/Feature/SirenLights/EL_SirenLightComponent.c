...
class EL_SirenLightComponent : ScriptComponent
{
	...
	void Play()
	{
		if (m_Anim && !m_Anim.IsPlaying()) m_Anim.Play();
	}

	//------------------------------------------------------------------------------------------------
	void Stop()
	{
		if (m_Anim && m_Anim.IsPlaying()) m_Anim.Stop();
	}
...
class EL_LightChild
{
	...
	//! \param parent The parent of the spawned light
	void Spawn(IEntity parent)
	{
		...
		LightFlags flags = LightFlags.CASTSHADOW | LightFlags.DIFFUSE_ONLY;
		vector color = m_vLightColor;
		color.Normalize();
