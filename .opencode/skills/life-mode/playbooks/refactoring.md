...
3. Prefer the Workbench refactor tools (`refactor-rename`, `refactor-replace-guid`, `refactor-move-resource`, and `refactor-remove-unused`) over hand-editing a reference tree. If no tool fits, use `rg` over the repo and Workbench resource paths. Never hand-edit a GUID across a tree.
...
6. Migrate callers and delete the legacy shape in the same wave. Do not leave a compatibility shim behind unless save data or saved-world references require it; make that migration explicit and delete the alias once no live reference remains.
